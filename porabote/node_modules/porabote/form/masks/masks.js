class Masks {
  ...
  timeRange = (value, e) => {
    if (e.nativeEvent.inputType == 'deleteContentBackward') return value;
    return this.setByPattern(value, '##:##-##:##');
  }

  setByPattern = (value, pattern) => {

    let i = 0;
    const v = this.digitalOnly(value).toString().padEnd(pattern.length, ' ');

    let newValue = pattern.replace(/#/g, _ =>  v[i++]);

    let returnedLength = (pattern[value.length] && pattern[value.length] != '#') ? value.length + 1 : value.length;

    return newValue.substring(0, returnedLength);
