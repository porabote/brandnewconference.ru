...
const Feed = (props) => {
  ...
  const {title, filter, data, meta} = useSelector(state => state.timings);
    ...
    >
      <div className="content feed">
        ...
        <div className="content__body">
          ...
          <Grid grid-template-columns="60px 300px 1fr 1fr">

            <div className="head">
              <div>ID</div>
              <div>Период проведения</div>
              <div>Заголовок для общего списка</div>
              <div>Заголовок для плеера</div>
            </div>

            {
             ...
             //   const rels = record.relationships;
                ...
                return (
                  <div linkTo={`/timings/view/${attrs.id}`} key={index}>
                    <div>{attrs.id}</div>
                    <div><b>{attrs.datetime_from} - {attrs.datetime_to}</b></div>
                    <div>{attrs.desc_list}</div>
                    <div>{attrs.desc_player}</div>
                  </div>
                )
              })
            }
