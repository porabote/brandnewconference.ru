...
const ViewContainer = (props) => {
  ...
  const {relationships} = useSelector(state => state.speakers);

  const getRecord = () => {

    const {id} = props;

    Api.get(`/api/speakers/get/${id}/`, {
      query: {
  ...
  const deleteRecord = (id, fetchFeedData) => {
    Api.get(`/api/speakers/method/delete/?id=${id}`)
      .then((resp) => {
        fetchFeedData();
        history.push('/speakers/feed/');
    });
  }

  const editRecordConfirm = (values, modalKey, getRecord) => {
    Api.post(`/api/speakers/method/edit/`, {
      body: values
    }).then((resp) => {
