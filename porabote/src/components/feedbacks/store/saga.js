...
function* fetchFeedDataAsync() {
  ...
  try {
    const data = yield call(() => {
      ...
      return Api.get(`/api/feedbacks/get/`, {
        query: {
          // where: where,
          // whereIn: state.mails.filter.whereIn,
          orWhereGrouped: state.feedbacks.filter.orWhereGrouped,
       //   include: state.mails.relationships,
          page: state.feedbacks.meta.nextPage,
          limit: state.feedbacks.meta.limit,
          orderBy: {
            id: 'DESC'
          }
        }
...
export default feedbacksWatcher;