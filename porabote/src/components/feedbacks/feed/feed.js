...
const Feed = (props) => {
  ...
  const {title, filter, data, meta} = useSelector(state => state.feedbacks);
    ...
    >
      <div className="content feed">
        ...
        <div className="content__body">
          ...
          <Grid grid-template-columns="60px 350px 350px 1fr">
...

            {
              data.map((record, index) => {
                ...
                return (
                  <div linkTo={`/feedbacks/view/${attrs.id}`} key={index}>
                    <div>{attrs.id}</div>
                    <div><b>{attrs.name}</b></div>
                    <div><b>{attrs.email}</b></div>
                    <div>{attrs.question}</div>
                  </div>
                )
              })
            }
