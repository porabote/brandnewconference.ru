...
const ViewContainer = (props) => {
  ...
  const { dictsRequired, title, meta, filter } = useSelector(state => state.hashes);
  const { components, dicts } = useSelector(state => state.dicts);

  const isDictsLoaded = components.hashes ? true : false;
  ...
  useEffect(() => {
    ...
    dispatch(requestDicts(dictsRequired, 'hashes'));
    getRecord();
  }, [props.id]);

  const {relationships} = useSelector(state => state.hashes);

  const getRecord = () => {

    const {id} = props;

    Api.get(`/api/hashes/get/${id}/`, {
      query: {
  ...
  const checkEditAccess = () => {
    Api.get(`/api/hashes/method/checkEditAccess/`)
      .then((resp) => {
  ...
  const editRecordConfirm = (values, modalKey, getRecord) => {
    Api.post(`/api/hashes/method/edit/`, {
      body: values
    }).then((resp) => {
