...
const Feed = (props) => {
  ...
  const {title, filter, data, meta} = useSelector(state => state.hashes);
    ...
    >
      <div className="content feed">
        ...
        <div className="content__body">
          ...
          <Grid grid-template-columns="60px 100px 1fr">
...

            {
              data.map((record, index) => {
                ...
                return (
                  <div linkTo={`/hashes/view/${attrs.id}`} key={index}>
                    <div>{attrs.id}</div>
                    <div>{attrs.part_format}</div>
                    <div><b>{attrs.hash}</b></div>
                  </div>
                )
              })
            }
