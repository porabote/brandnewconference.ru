...
const ViewContainer = (props) => {
  ...
  const { dictsRequired, title, meta, filter } = useSelector(state => state.textBoxes);
  const { components, dicts } = useSelector(state => state.dicts);

  const isDictsLoaded = components.textBoxes ? true : false;
  ...
  useEffect(() => {
    ...
    dispatch(requestDicts(dictsRequired, 'textBoxes'));
    getRecord();
  }, [props.id]);

  const {relationships} = useSelector(state => state.textBoxes);

  const getRecord = () => {

    const {id} = props;

    Api.get(`/api/textBoxes/get/${id}/`, {
      query: {
  ...
  const checkEditAccess = () => {
    Api.get(`/api/textBoxes/method/checkEditAccess/`)
      .then((resp) => {
  ...
  const editRecordConfirm = (values, modalKey, getRecord) => {
    Api.post(`/api/text-boxes/method/edit/`, {
      body: values
    }).then((resp) => {
