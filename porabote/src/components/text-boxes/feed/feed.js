...
const Feed = (props) => {
  ...
  const {title, filter, data, meta} = useSelector(state => state.textBoxes);
          ...
          <Grid grid-template-columns="60px 300px 1fr">
...

            {
              data.map((record, index) => {
                ...
                return (
                  <div linkTo={`/text-boxes/view/${attrs.id}`} key={index}>
                    <div>{attrs.id}</div>
                    <div><b>{attrs.desc}</b></div>
                    <div>{attrs.box_text}</div>
                  </div>
                )
              })
            }
